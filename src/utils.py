...
# class elemw_multiplyer(nn.Module):
#     def __init__(self,w,h):
#         super().__init__()

#         self.fake_conv = nn.ConvTranspose2d(in_channels = 1, out_channels =1, kernel_size = (w,h), groups=1, bias=False)    
...
def compute_pred(X_ini, S_m, N_m, w_s, w_n):

    S_pred = S_m + w_s*(X_ini - (S_m+N_m))
    N_pred = N_m + w_n*(X_ini - (S_m+N_m)) 
    #N_pred = N_m + w_n*(X_ini - (S_pred+N_m))        
    X_pred = (S_pred + N_pred)

    return S_pred, N_pred, X_pred
...
class elemw_multiplyer(nn.Module):
    def __init__(self,w,h):
        ...
        # self.fake_conv = nn.ConvTranspose2d(in_channels = 1, out_channels =1, kernel_size = (w,h), groups=1, bias=False)
        # self.fake_conv.weight.data = torch.ones(w,h)

        self.W = nn.Parameter(torch.ones(w,h), requires_grad=True)



    def forward(self, x, X_mix, S_pred, N_pred):
        # w_speech = F.sigmoid(self.fake_conv(x))

        w_speech = F.sigmoid(self.W)

        w_noise = 1 - w_speech  

        S_pred, N_pred, X_pred = compute_pred(X_mix, S_pred, N_pred, w_speech, w_noise)

        return S_pred.detach(), N_pred.detach(), X_pred
