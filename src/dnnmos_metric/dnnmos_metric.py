...
COEFS_SIG = np.array(
    [9.651228012789436761e-01, 6.592637550310214145e-01, 7.572372955623894730e-02]
)
COEFS_BAK = np.array(
    [-3.733460011101781717e00, 2.700114234092929166e00, -1.721332907340922813e-01]
)
COEFS_OVR = np.array(
    [8.924546794696789354e-01, 6.609981731940616223e-01, 7.600269530243179694e-02]
)
...
def compute_dnsmos(audio, fs=16000, input_length=9, _sessions={}):
    ...
    for idx in range(num_hops):
        ...
        onnx_inputs_sig = {
            inp.name: input_features for inp in _sessions["sig"].get_inputs()
        }
        mos_sig = poly.polyval(_sessions["sig"].run(None, onnx_inputs_sig), COEFS_SIG)

        onnx_inputs_bak_ovr = {
            inp.name: input_features for inp in _sessions["bak_ovr"].get_inputs()
        }
