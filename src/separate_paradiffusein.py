...
@InferenceAlgoRegistry.register("separate_paradiffusein")
class SeparateParaDiffUSEIN:
	def __init__(
		self,
	...
	def predictor_corrector(self, St, t, laststep, dt, noise=False, snr=0.5, v=None):
		...
		# Corrector  
		with torch.no_grad():            
			score = score_model.forward(St, t, v)
			step_size = (snr * std) ** 2
	...
	def likelihood_update_individual(self, St, S0hat, t, std, std_noise, dt, lmbd, w_up=True,v=None):
		"""
		...
		with torch.no_grad():
			...
			score_noise = score_model.forward(self.X - (St / mu_t) - (std[:, None, None, None] / mu_t) * z, t, v)
	...
	def posterior_sampler(self, startstep=0, S0=None, N0=None):  
		...
		for i in range_i:
			...
			if self.print_progress:
				...
				metrix = calc_metrics(self.s_ref.numpy().reshape(-1), st)
				print(
