...
ResnetBlockDDPM = layerspp.JointResnetBlockDDPMpp
ResnetBlockBigGAN = layerspp.JointResnetBlockBigGANpp
...
@BackboneRegistry.register("jointncsnpp")
class JointNCSNpp(nn.Module):
    """Label conditionned NCSN++ model, adapted from SGMSE+ repository"""
    ...
    def __init__(self,
        ...
        joint_noise_clean_speech_training = True,       
        **unused_kwargs
    ):
        ...
        assert (self.audio_only and self.vfeat_processing_order == "default" and joint_noise_clean_speech_training)
        ...
        self.label_emb = nn.Embedding(2, nf * 4) #nn.Embedding(num_classes, time_embed_dim): https://github.com/openai/guided-diffusion/blob/22e0df8183507e13a7813f8d38d51b072ca1e67c/guided_diffusion/unet.py#L478
    ...
    def forward(self, x, time_cond):
        ...
        #remember x is a list : [stft, label]"
        ...
        # Convert real and imaginary parts of x into 2 channel dimensions
        else:            
            x = torch.cat((x[0][:,[0],:,:].real, x[0][:,[0],:,:].imag), dim=1) 
        ...
        if self.conditional:
            ...
            temb = temb + self.label_emb(label) # https://github.com/openai/guided-diffusion/blob/22e0df8183507e13a7813f8d38d51b072ca1e67c/guided_diffusion/unet.py#L652
...
@BackboneRegistry.register("jointncsnpp28M")
class JointNCSNpp28M(JointNCSNpp):
    """Medium-scale JointNCSN++ model. ~28M parameters"""
...
@BackboneRegistry.register("jointncsnpp12M")
class JointNCSNpp12M(JointNCSNpp):
    """Small-scale JointNCSN++ model. ~12M parameters"""
...
@BackboneRegistry.register("jointncsnpp6M")
class JointNCSNpp6M(JointNCSNpp):
    """Tiny-scale JointNCSN++ model. ~6M parameters"""
