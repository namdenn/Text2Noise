...
from .ncsnpp_utils_flow_avse import layers, layerspp, normalization
...
from .attention_flow_avse import SpatialTransformer
...
class visualFrontend(pl.LightningModule): # Activated visualTCN & Visualconv1d
	...
	def forward(self, inputBatch):
		...
		batch = batch.reshape(batch.shape[0]*batch.shape[1], batch.shape[2], batch.shape[3], batch.shape[4]).contiguous() # 600,64,28,28
...
@BackboneRegistry.register("ncsnpp_flow_avse")
class NCSNpp_flow_avse(nn.Module):
	...
	def __init__(self, 
		...
		transformer_depth = 1,
		context_dim = 64, # 임시로 설정해준 값임, original 32
		#####################################
		...
		**kwargs):
		...
		self.video_feature_type = kwargs["video_feature_type"]
	...
	def forward(self, x, time_cond=None, v=None):
		...
		#else:
		context = self.visual_encoder(v) #context = [8,75,112,112]
