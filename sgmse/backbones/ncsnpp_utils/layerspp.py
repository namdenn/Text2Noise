...
class JointResnetBlockDDPMpp(nn.Module):
    """ResBlock adapted from DDPM."""
    ...
    def __init__(
        ...
        self.Conv_0 = conv3x3(in_ch, out_ch)
        if temb_dim is not None:
            temb_out_ch = out_ch if cemb_dim is not None else 2 * out_ch
            self.Dense_0 = nn.Linear(temb_dim, temb_out_ch)
            self.Dense_0.weight.data = default_init()(self.Dense_0.weight.data.shape)
            nn.init.zeros_(self.Dense_0.bias)
        if cemb_dim is not None:
            self.Dense_1 = nn.Linear(cemb_dim, 2 * out_ch)
            try:
                nn.init.variance_scaling_(self.Dense_1.weight, scale=0.02, mode='fan_in', distribution='uniform')
            except AttributeError:
                nn.init.xavier_uniform_(self.Dense_1.weight, gain=0.02)
    ...
    def forward(self, x, temb=None, cemb=None):
        h = self.act(self.GroupNorm_0(x))
        h = self.Conv_0(h)
        if self.cemb_dim is None:
            if temb is not None:
                emb_out = self.Dense_0(self.act(temb))
                scale, shift = torch.chunk(emb_out, 2, dim=1)
                h = self.GroupNorm_1(h) * (1 + scale)[:, :, None, None] + shift[:, :, None, None]
            else:
            ...
            h = self.GroupNorm_1(h)
            if cemb is not None:
                emb_out = self.Dense_1(self.act(cemb))
                scale, shift = torch.chunk(emb_out, 2, dim=1)
...
class JointResnetBlockBigGANpp(nn.Module):
    def __init__(
        self,
        act,
        in_ch,
