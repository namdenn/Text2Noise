...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="count number of frames (on slurm)",
    ...
    jobs = executor.map_array(
        check, fids_arr, [audio_dir for _ in fids_arr], [video_dir for _ in fids_arr]
    )
    ...
    if len(missing_fids) > 0:
        ...
        jobs = executor.map_array(
            count_frames,
            fids_arr,
            [audio_dir for _ in fids_arr],
            [video_dir for _ in fids_arr],
        )
