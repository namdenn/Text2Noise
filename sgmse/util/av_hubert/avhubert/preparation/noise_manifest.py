    ...
    for category in ["babble", "music", "noise"]:
        ...
        if category in {"music", "noise"}:
            ...
            train_fns, valid_fns, test_fns = (
                target_fns[:num_train],
                target_fns[num_train : num_train + num_valid],
                target_fns[num_train + num_valid :],
            )
...
def combine(input_tsv_dirs, output_dir):
    ...
    for input_tsv_dir in input_tsv_dirs:
        train_fn, valid_fn, test_fn = (
            [ln.strip() for ln in open(f"{input_tsv_dir}/train.tsv").readlines()],
            [ln.strip() for ln in open(f"{input_tsv_dir}/valid.tsv").readlines()],
            [ln.strip() for ln in open(f"{input_tsv_dir}/test.tsv").readlines()],
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Set up noise manifest",
    ...
    print(f"Combine tsv")
    input_tsv_dirs = [f"{output_tsv_dir}/{x}" for x in ["noise", "music", "babble"]] + [
        f"{args.lrs3}/noise/speech"
    ]
