        ...
        for txt_fn in txt_fns:
            ...
            for i_word, (word, start, end) in enumerate(word_intervals):
                if i_word == 0:
                    ...
                    assert (
                        start >= cur_sent[-1][-1]
                    ), f"{fid} , {word}, start-{start}, prev-{cur_sent[-1][-1]}"
...
def trim_video_frame(csv_fn, raw_dir, output_dir, ffmpeg, rank, nshard):
    ...
    for raw_fid, fid_info in tqdm(fid_info_shard):
        ...
        for fid, start_sec, end_sec in fid_info:
            ...
            if end_sec == -1:
                end_sec = 24 * 3600
            start_frame_id, end_frame_id = int(start_sec * fps), min(
                int(end_sec * fps), num_frames
            )
            imnames = [
                tmp_dir + "/" + str(x + 1).zfill(decimal) + ".png"
...
def trim_audio(csv_fn, raw_dir, output_dir, ffmpeg, rank, nshard):
    ...
    for raw_fid, fid_info in tqdm(fid_info_shard):
        ...
        cmd = (
            ffmpeg
            + " -i "
            + os.path.join(raw_dir, raw_fid + ".mp4")
            + " -f wav -vn -y "
            + wav_path
            + " -loglevel quiet"
        )
...
def trim_pretrain(root_dir, ffmpeg, rank=0, nshard=1, step=1):
    ...
    if step == 1:
        ...
        output_video_dir, output_audio_dir = os.path.join(
            root_dir, "short-pretrain"
        ), os.path.join(root_dir, "audio/short-pretrain/")
...
def prep_wav(lrs3_root, ffmpeg, rank, nshard):
    ...
    video_fns = glob.glob(lrs3_root + "/trainval/*/*mp4") + glob.glob(
        lrs3_root + "/test/*/*mp4"
    )
...
def get_file_label(lrs3_root):
    ...
    for video_id, label in zip(df["id"], df["text"]):
        ...
        labels_total.append(label)
    video_id_fn, label_fn = os.path.join(lrs3_root, "file.list"), os.path.join(
        lrs3_root, "label.list"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="LRS3 preprocess pretrain dir",
    ...
    parser.add_argument(
        "--step",
        type=int,
        help="Steps (1: split labels, 2: trim video/audio, 3: prep audio for trainval/test, 4: get labels and file list)",
    )
