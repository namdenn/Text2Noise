...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="LRS3 tsv preparation",
    ...
    assert os.path.isfile(
        file_list
    ), f"{file_list} not exist -> run lrs3_prepare.py first"
    assert os.path.isfile(
        label_list
    ), f"{label_list} not exist -> run lrs3_prepare.py first"
    nframes_audio_file, nframes_video_file = (
        f"{args.lrs3}/nframes.audio",
        f"{args.lrs3}/nframes.video",
    )
    assert os.path.isfile(
        nframes_audio_file
    ...
    fids, labels = [x.strip() for x in open(file_list).readlines()], [
        x.strip().lower() for x in open(label_list).readlines()
    ]
