...
def split_musan(musan_root, rank, nshard):
    wav_fns = (
        glob.glob(f"{musan_root}/speech/*/*wav")
        + glob.glob(f"{musan_root}/music/*/*wav")
        + glob.glob(f"{musan_root}/noise/*/*wav")
    )
...
def get_speaker_info(musan_root):
    ...
    train_speakers, valid_speakers, test_speakers = (
        speakers[:num_train],
        speakers[num_train : num_train + num_valid],
        speakers[num_train + num_valid :],
    )
...
def make_musan_babble(musan_root, rank, nshard):
    ...
    for split in ["train", "valid", "test"]:
        speakers = [
            ln.strip() for ln in open(f"{musan_root}/speech/spk.{split}").readlines()
        ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="MUSAN audio preparation",
    ...
    parser.add_argument(
        "--slurm_partition", type=str, default="cpu", help="slurm partition"
    )
    ...
    jobs = executor.map_array(
        split_musan, [args.musan for _ in ranks], ranks, [args.nshard for _ in ranks]
    )
    ...
    jobs = executor.map_array(
        make_musan_babble,
        [short_musan for _ in ranks],
        ranks,
        [args.nshard for _ in ranks],
    )
    [job.result() for job in jobs]
    print(f"Count number of frames")
    wav_fns = (
        glob.glob(f"{short_musan}/babble/*/*wav")
        + glob.glob(f"{short_musan}/music/*/*wav")
        + glob.glob(f"{short_musan}/noise/*/*wav")
    )
    jobs = executor.map_array(
        count_frames, [wav_fns for _ in ranks], ranks, [args.nshard for _ in ranks]
    )
