...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generating babble and speech noise from LRS3",
    ...
    output_tsvs = [
        os.path.join(args.lrs3, "noise", "babble", "valid.tsv"),
        os.path.join(args.lrs3, "noise", "babble", "test.tsv"),
    ]
    ...
    wav_fns = [
        (ln.strip().split("\t")[2], int(ln.strip().split("\t")[-1])) for ln in lns
    ]
    ...
    speech_tsv_dir, speech_wav_dir = os.path.join(
        args.lrs3, "noise", "speech"
    ), os.path.join(args.lrs3, "noise", "speech", "wav")
    ...
    num_train, num_valid, num_test = (
        int(len(noise_fns) * 0.6),
        int(len(noise_fns) * 0.2),
        int(len(noise_fns) * 0.2),
    )
    prev = 0
