...
def add_task_state(ckpt_path):
    ...
    if cfg["model"]["_name"] == "av_hubert":
        dictionaries = [
            Dictionary.load(f"{cfg['task']['label_dir']}/dict.{label}.txt")
            for label in cfg["task"]["labels"]
        ]
        ...
        dictionaries = [
            Dictionary.load(f"{prt['cfg']['task']['label_dir']}/dict.{label}.txt")
            for label in prt["cfg"]["task"]["labels"]
        ]
