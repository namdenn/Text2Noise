...
def main():
    parser = argparse.ArgumentParser(
        description="clustering", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    ...
    parser.add_argument(
        "--nlayer", type=int, default=12, help="layer index for clustering"
    )
    ...
    parser.add_argument(
        "--percent", type=float, default=0.05, help="Percentage for clustering"
    )
    ...
    if is_mfcc:
        ...
        for rank in range(nshard):
            ...
            args_array.append(args)
        args_array.append(
            [tsv_dir, "valid", ckpt_path, nlayer, 1, 0, output_dir, 1600000]
        )
        ext.update_parameters(
            timeout_min=60,
            slurm_partition=slurm_partition,
    ...
    args, kwargs = [feat_dir, "train", nshard, km_path, n_clusters], vars(
        Namespace(
            seed=0,
            percent=percent,
            init="k-means++",
