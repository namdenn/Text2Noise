...
class OUVESDE(SDE):
    ...
    ## from https://github.com/sp-uhh/derevdps/blob/main/sgmse/sdes.py#L189
    def tweedie_from_score(self, score, xt, t, *args):
        sigma = self._std(t)
        sigma = sigma.view(sigma.size(0), *(1,)*(score.ndim - sigma.ndim))
        return xt + sigma**2 * score
...
class OUVPSDE(SDE):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument(
            "--beta-min", type=float, default=1e-4,  help="The minimum beta to use." ##modified 08th of june 2024
        )
        parser.add_argument(
            "--beta-max", type=float, default=0.02,  help="The maximum beta to use." ##modified 08th of june 2024
        )
        parser.add_argument(
            "--stiffness",
            type=float,
...
@SDERegistry.register("ot_flow")
class OTFlow(SDE):
    """Optimal Transport Flow Matching (FM-OT) Probability Path.

    Linear interpolation: x_t = (1 - (1 - sigma_min)*t) * x_0 + t * x_1
    Velocity vector field: u_t = x_1 - (1 - sigma_min) * x_0
    """
    ...
    def add_argparse_args(parser):
        parser.add_argument(
            ...
            type=int,
            default=20,
            help="Number of ODE solver integration steps during sampling (20 by default).",
        )
        parser.add_argument(
            "--sigma-min",
            type=float,
            default=1e-4,
            help="Minimum noise scale at t=0 for numerical stability (1e-4 by default).",
        )
        return parser

    def __init__(self, sigma_min=1e-4, sde_n=20, N=20, **kwargs):
        n_steps = sde_n if sde_n is not None else N
        super().__init__(n_steps)
        self.sigma_min = sigma_min
        self.N = N
        self.__name__ = "ot_flow"

    def copy(self):
        return OTFlow(sigma_min=self.sigma_min, N=self.N)
    ...
    def marginal_prob(self, x1, t):
        """Computes OT path interpolation given target x1 and continuous time t.

        Note: Unlike SDEs that compute mean/std for Gaussian perturbations,
        Flow Matching constructs a deterministic path between x0 ~ N(0, I) and
        x1.
        """
        t_b = (
            t.view(-1, 1, 1, 1)
            if x1.ndim == 4
            else t.view(t.size(0), *(1,) * (x1.ndim - 1))
        )
        x0 = torch.randn_like(x1)

        # OT path interpolation: xt
        xt = (1.0 - (1.0 - self.sigma_min) * t_b) * x0 + t_b * x1
        # Target velocity field: ut
        ut = x1 - (1.0 - self.sigma_min) * x0

        return xt, ut

    def sde(self, x, t):
        """Returns dummy drift and zero diffusion because Flow Matching is a

        deterministic ODE (Probability Flow), not a stochastic SDE.
        """
        drift = torch.zeros_like(x)
        diffusion = torch.zeros((x.shape[0],), device=x.device)
        return drift, diffusion

    def prior_sampling(self, shape, device):
        """Generates pure Gaussian noise x_0 ~ N(0, I) at time t=0."""
    ...
    def discretize(self, x, t):
        """Euler discretization step size dt = 1/N for ODE sampling."""
        dt = 1.0 / self.N
        return dt, torch.zeros((x.shape[0],), device=x.device)